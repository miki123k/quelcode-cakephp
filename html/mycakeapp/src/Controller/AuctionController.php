			...
			if ($img_data['error'] !== 0) {
				...
				if (in_array($img_type, $permit_type, true)) {
					...
					if ($this->Biditems->save($biditem)) {
						...
						//img/auctionに保存
						move_uploaded_file($img_data['tmp_name'], 'img/' . $path);
						//データベースのimage_pathを更新して保存する
						$biditem['image_path'] = $path;
						$this->Biditems->save($biditem);
						// 成功時のメッセージ
					...
					//画像の拡張子が$permit_type以外ならエラーメッセージを表示
					$this->Flash->error(__('jpg,png,gif形式のみアップロード可能です'));
				}
			}
		}
		// 値を保管
